from water_flow import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe, pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction, convert_kilopascals_to_psi
...
#Here is the exceeding requirements I made, the expected result was calculated first by me and I applied in the test function
def test_convert_kilopascals_to_psi():
    assert convert_kilopascals_to_psi(158.7) == approx(23.02, abs=1)
    assert convert_kilopascals_to_psi(100) == approx(14.51, abs=1)
    assert convert_kilopascals_to_psi(30) == approx(4.35, abs=1)
